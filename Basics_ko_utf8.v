...
(** 함수형 프로그래밍 스타일은 간단한, 매일 사용하는 수학적 직관에
    ...
    _1등급(first-class)_ 값들로 사용하는 것을 강조한다는 의미로도
...
(** 이 타입을 [day]이라 부르고, 각 멤버들은 [monday], [tuesday]
    등이다.  정의에서 두 번째 줄과 그 이후를 "[monday]는 [day]이고,
    [tuesday]는 [day]이고, 등등"과 같이 읽는다.

    [day]를 정의했으면 요일들을 다루는 함수들을 작성할 수 있다. *)
...
(** 함수를 정의하면 몇 가지 예제들을 가지고 잘 동작하는지 확인해야
    ...
    제공한다. 첫째, [Compute] 명령어로 [next_weekday]를 사용하는
    복합식을 계산할 수 있다. *)
...
(** (주석에 이 계산 결과를 작성해놓았다. 컴퓨터를 가지고 있다면 당신이
    ...
    파일들에서 이 파일 [Basics.v]을 불러와서 위 예제를 찾아, 콕에
    명령을 내리고 결과를 살펴보자.)
...
(** 이 선언에서 두 가지를 수행한다. ([saturday] 다음 두 번째 평일이
    [tuedday])라는 주장을 만들고, 나중에 이 주장을 참조하는데 사용할
...
(** 증명에서 사용한 자세한 사항들은 당장 중요하지 않다(곧 설명할
    ...
    셋째, 콕으로 하여금 우리가 만든 [정의]로 부터 고성능 컴파일러를
...
(** 비록 처음부터 모든 것을 만들어 보이기 위해서 우리만의 부울값들을
    ...
    라이브러리 문서에서 [Coq.Init.Datatypes]을 살펴보라.) 가능한
...
(** 마지막 두 개는 다중 인자 함수 정의들에 대한 콕 구문을 설명하고
    ...
    설명한다. 이 테스트들은 [orb] 함수에 대한 완전한 명세, 진리표를
    구성한다. *)
...
(** _표기법에 대한 언급_: [.v] 파일들에서 주석 안에 대괄호들을
    사용하여 콕 코드 일부들을 구분한다. 이 규칙은 [coqdoc] 문서
    도구에서도 사용하는데 주변 텍스트로 부터 코드들을 시각적으로
    구분되도록 한다. 이 파일들을 변환한 html 버전에서는 [다른 폰트]로
    보일 것이다.

    [Admitted] 명령어는 증명이 완전하지 않을 때 나중에 채워넣을 수
    있는 공간으로 사용할 수 있다. 연습문제들에 이 명령어를 사용해서
    독자들이 실제 증명들로 대체할 부분임을 나타낸다. *)

(** **** 연습문제: 별 1개 (nandb)  *)

(** "[Admitted]"를 제거하고 다음 함수의 정의를 완성하시오. 그런 다음
    아래의 [Example] 주장들을 콕으로 각각 검증할 수 있음을
    확인하시오. ("[Admitted]"를 제거하고 각 증명을 채워 넣으시오.
    위의 [orb] 테스트들의 모델을 따른다.) 이 함수는 입력들 중 하나
    또는 둘 다 [false]이면 [true]를 리턴해야 한다. *)

Definition nandb (b1:bool) (b2:bool) : bool
  (* 이 줄을 ":= _정의_ ."로 바꾸시오. *). Admitted.
...
(** **** 연습문제: 별 1개 (andb3)  *)
(** 아래 [andb3] 함수에 대해서도 풀어 보시오. 이 함수는 모든 입력들이 [true]이면
    [true]를 반환하고 그렇지 않으면 [fasle]를 반환한다. *)
...
(** 콕의 모든 식은 타입이 매겨져 있어서 식의 계산 결과를
    나타낸다. [Check] 명령으로 식의 타입을 출력할 수 있다.  *)
...
(** [negb] 같은 함수들은 그 자체가 마치 [true]와 [false] 같은 데이터
...
(** [negb]의 타입은 [bool -> bool]라고 작성하고 "[bool] 애로우
    [bool]"이라 발음하고 "[bool] 타입의 입력이 주어지면 [bool] 타입의
    출력을 내는 함수이다."라고 읽는다. 유사하게 [andb]의 타입은 [bool
    -> bool -> bool]이라고 작성하고 "[bool] 타입인 두 입력들이
    주어지면 [bool] 타입의 출력을 내는 함수이다"라고 읽는다. *)
...
(** 콕은 _모듈 시스템_을 제공하여 큰 규모의 개발을 구성하는 것을
    ...
    것이지만 한 가지는 유용하다. [Module X]와 [End X] 표시들 사이에
    선언들을 감싸면 [End] 다음에서 이 정의들을 참조하려면 [X.foo]와
    같은 이름들을 사용한다. 이 특징을 이용하여 내부 모듈에 [nat]
...
(** 이 정의의 각 절들을 다음과 같이 볼 수 있다.
     - [O]는 자연수이다. (숫자 "[0]"가 아니라 문자 "[O]"입니다.)
     - [S]는 자연수 앞에 놓아서 또 다른 자연수를 만들 수 있습니다. 즉
       [n]이 자연수이면 [S n]도 자연수입니다. *)

(** 조금 더 자세히 이 타입을 살펴봅시다.

    모든 귀납적으로 정의한 집합 ([day], [nat], [bool], 등.)은 사실
    [O], [S], [true], [false], [monday], 등과 같은 _생성자들_로 부터
    만든 _식들_의 집합이다. [nat]의 정의는 집합 [nat]의 식들을 어떻게
    만드는지를 말한다.

    - [O]와 [S]는 생성자들이다.
    - 식 [O]는 집합 [nat]에 속한다.
    - 만일 [n]이 집합 [nat]에 속하는 식이라면 [S n]도 이 집합에 속하는
      식이다. 그리고,
    - 이렇게 두 가지 방법으로 형성된 식들만 집합 [nat]에 속한다.  *)

(** [day]와 [bool]을 정의한 것에 대해 동일한 규칙들을
    적용한다. (생성자들에 대해 사용했던 주석들은 [O] 생성자에 대한
    주석과 유사하게 인자를 받지 않는다.)

    위 조건들은 [Inductive] 선언의 정확한 힘이다. 이 조건들은 식 [O],
    식 [S O], 식 [S (S O)], 식 [S (S (S O))], 등등이 집합 [nat]에
    속한다는 것을 뜻하고, 예를 들어 [true], [andb true false], [S (S
    false)], [O (O (O S))]와 같은 다른 식들은 집합 [nat]에 속하지
    않는다는 것을 의미한다.

    여기서 중요한 점은 이제까지 우리가 한 것은 단지 숫자들의
    _표현_(숫자들을 쓰는 방법)을 정의한 것뿐이다. [O]와 [S]는 임의로
    정한 이름들이고, 지금은 특별한 의미를 가지고 있지 않다. 이
    이름들은 단지 두 가지 다른 표시들로 숫자들을 쓸 때 사용할 수
    있다. 물론 [nat]의 원소는 [O] 다음에 [S] 표시들로 감싸는
...
(** 두 번째 분기는 "만일 [n]이 어떤 [n']에 대해 [S n'] 형태라면
    이 함수는 [n']을 리턴한다."로 해석할 수 있다. *)
...
(** 자연수는 아주 흔하게 사용하기 때문에 콕은 자연수들을 파싱하고
    출력하는 약간의 기능을 제공하는데, 일반 아라비아 숫자들은 [S]와
    [O] 생성자들로 정의하는 "단항" 표기법의 대안으로 사용할 수
...
(** 마치 함수들 [minustwo]와 [pred]와 같이 생성자 [S]도 [nat -> nat]
    타입을 갖는다. *)
...
(** 이것들은 모두 숫자에 적용해서 숫자를 반환한다. 하지만 첫 번째와
    다른 두 가지 사이에 근본적인 차이가 있다. [pred]와 [minustwo] 같은
    함수들은 _계산 규칙들_와 함께 온다. 예를 들어 [pred]의 정의에 의해
    [pred 2]가 [1]로 계산된다.  [S]의 정의에는 이러한 것이 달려있지
    않다. 인자에 적용될 수 있다는 면에서 함수와 비슷하지만 아무런
    계산을 _하지_ 않는다! 단지 숫자를 쓰는 방법이다. (표준 아라비아
    숫자들에 대해 생각해보자. 숫자 [1]은 계산이 아니고 하나의
    데이터이다. [111]이라고 쓸 때 숫자 백십일을 의미할 때 [1]을 세 번
    사용해서 어떤 숫자를 구체적으로 표현한다.)

    숫자들에 대한 대부분의 함수 정의들에 대해 패턴 매칭만으로 충분하지
    않고 재귀가 필요하다. 예를 들어 숫자 [n]이 짝수인지 보려면
    재귀적으로 [n-2]가 짝수인지 확인할 필요가 있을 수 있다. 이런
    함수들을 쓰려면 키워드 [Fixpoint]를 사용한다. *)
...
(** [oddb]도 비슷하게 [Fixpoint]를 사용한 선언으로 정의할 수 있지만
    여기서 더 간단하게 정의한다. *)
...
(** (이 증명들을 따라가보면 [simpl]은 사실 증명 목표를 달성하기 위해
    아무런 효과를 내지 못한다. 모든 것은 [reflexivity]에 의해
...
(** 편리한 표기법으로 둘 이상의 인자들이 같은 타입이면 한 번에 작성할
    수 있다.  다음 정의에서 [(n m : nat)]은 [(n : nat) (m : nat)]으로
    작성하는 것과 동일하다. *)
...
(** ([level], [associativity], [nat_scope] 표기법들은 콕의 파서가 이
    ...
    [plus x y] 대신에 [x + y]를 이해하도록 하는 명령어들이고, 또한 
    역으로 콕 출력기가 [plus x y] 대신 [x + y]를 출력하게 한다. *)

(** 콕에서 처음부터 내장된 것이 거의 없다고 얘기했을 때, 정말로
    그렇다.  심지어 숫자들이 같은지 테스트하는 것조차 사용자 정의
    연산이다!  이제 함수 [beq_nat]를 정의해서 [nat]의 자연수들이 [eq]
    같은지 테스트하고 [b] 부울 값을 낸다. 중첩된 [match]를 사용한다. (
    [minus]에서처럼 동시에 패턴 매치하는 방법을 사용할 수도
    있었다.) *)
...
(** [leb] 함수는 첫 번째 인자가 두 번째 인자와 같거나 작은지 테스트하고
    부울 값을 낸다. *)
...
(** **** 연습문제: 별 하나 (blt_nat)  *)
(** [blt_nat] 함수는 [nat]의 자연수들을 비교해서 작은지 비교하고 ([l],
    [t]) 부울 값을 낸다. 새로운 [Fixpoint] 함수를 만들지 않고 이전에
    만든 함수에 관해서 정의한다. *)
...
(** 이제 몇 가지 자료형들과 함수들을 정의했으니 이들의 동작에 대한
    성질들을 기술하고 증명해보자. 사실 이전 절들의 각 [Example]에서
    어떤 함수와 어떤 입력들에 대한 동작에 대한 정밀한 주장을 이미
    만들기 시작해왔다. 이 주장들을 증명하는 것은 항상
    동일했다. [simpl]을 사용하여 등식의 양 편을 간략화하고
    [reflexivity]를 사용하여 양 쪽이 동일한 값인지 검사하였다.

    동일한 류의 "간략화에 의한 증명"은 더 흥미로운 성질들을
    증명하는 데에도 사용될 수 있다. 예를 들어, [0]은 [+]에 대한 왼편
    "중성 요소"라는 사실은 [n]이 무엇이든지 [0 + n]을 [n]으로 환원하는
    것을 관찰해서 증명할 수 있다. [plus] 정의로부터 직접 확인할 수
    있는 사실이기도 하다. *)
...
(** (위 문장은 통합개발환경에서 [.v] 파일에서 보는 것과 브라우저에서
    HTML로 보는 것이 다르다는 것을 인지 할 수 있다. [.v] 파일들은
    [forall] 전체 한정자를 예약 식별자 "forall"로 작성한다. [.v]
    파일들이 HTML로 변환될 때 이 식별자는 A를 상하로 뒤집은 기호로
    변환된다.)

    이제 [reflexivity]가 우리가 인정해온 것보다 더 강력하다는 것을
    언급하기에 적절하다. 우리가 봐온 예제들에서 [simpl] 명령어를
    사용하는 것은 사실 필요하지 않았었다. 왜냐하면 [reflexivity]로
    양쪽이 동일한지 검사할 때 자동으로 약간의 간략화를 수행하기
    때문이다. [simpl]을 추가해서 증명이 끝나기 전에 간략화한 다음의
...
(** 게다가 [reflexivity]는 [simpl]보다 다소 _더 많이_ 간략화를
    수행한다는 것을 아는 것이 나중에 유용할 것이다. 예를 들어, 정의된
    낱말들을 "펼치기"를 시도해서 이 낱말들을 오른편 정의들로 대체한다.
    이러한 차이에 대한 이유는 만일 [reflexivity]가 성공하면 전체
    목표를 달성하지만, [reflexivity]가 모든 이러한 간략화와 펼치기를
    통해 만든 확장된 식들이 무엇인지 들여다볼 필요가 없다. 반면에
    [simpl]은 새로 만든 목표가 무엇인지 보고 이해할 필요가 있는
    ...
    첫째, [Example] 대신 [Theorem] 키워드를 사용했다. 이 차이는 대부분
    스타일의 문제이다. [Example]과 [Theorem] 키워드들은 (그리고
    [Lemma], [Fact], [Remark]와 같은 몇 가지 더 다른 키워드들) 콕
    입장에서는 거의 같다.

    둘째, 정리들에서 이 정리들이 _모든_ 자연수들 [n]에 대해 얘기하기
    위해 사용한 한정자 [forall n:nat]를 추가하였다. 보통 이러한
    정리들을 증명하기 위해 일반적으로 "[n]은 어떤 숫자라고
    가정하고..."라고 얘기하면서 시작한다. 형식적인 증명에서는 [intros
    n]으로 시작한다. 이것은 증명 목표에 있는 그 한정자로부터 [n]을
    현재 가정하는 _문맥_으로 이동시키는 역할을 한다.

    키워드들 [intros], [simpl], [reflexivity]는 _전술들_
    예다. 전술이란 [Proof]와 [Qed] 사이에서 사용하는 명령으로 우리가
...
(** (왜 이러한 상황이 발생했는지 설명할 수 있나?  콕을 가지고 단계별로
    ...
    증명 도중에 막히면 [Abort] 명령을 사용해 잠시 증명을 포기할 수
    있다. *)
...
(** 모든 숫자들 [n]과 [m]에 대한 전체적 주장이 아니라 [n = m]이 성립할
    때의 더 특별한 성질에 대해 서술하고 있다. 화살표 기호는
    "함축한다"라고 읽는다.

    이전처럼 그러한 조건을 만족하는 숫자들 [n]과 [m]이 주어져 있다고
    가정하고 추론할 필요가 있다. [intros] 전술은 목표로부터 이 세 가지
    모두를 현재 문맥의 가정들로 옮기는 역할을 할 것이다.

    [n]과 [m]은 임의의 숫자들이기 때문에 이 정리를 증명하기 위해
    간략화 방법을 무조건 사용할 수는 없다. 대신에 [n = m]을 가정하면
    목표 문장에서 [n]을 [m]을 대체해서 양쪽에 동일한 식으로 구성된
    등식을 얻을 수 있다는 점을 이용해 증명한다. 콕이 이런 방법으로
    대체하는 과정을 수행하려면 [rewrite] 전술을 사용한다. *)
...
Proof.
  ...
  (* 가정을 현재 문맥으로 옮긴다: *)
  intros H.
  (* 가정을 사용하여 목표를 다시 작성한다: *)
  rewrite -> H.
  reflexivity.  Qed.

(** 증명의 첫 번째 줄에서 전체 한정된 변수들 [n]과 [m]을 현재 문맥으로
    옮긴다.  두 번째 줄에서 가정 [n = m]을 이 문맥으로 옮기고 [H]라고
    이름 붙인다.  세 번째 줄은 현재 목적 ([n + n = m + m])을 다시
    작성하여 등식 형태의 가정 [H]의 왼편을 오른편으로 대체하도록
    콕에게 명령한다.

    ([rewrite]의 화살표는 논리적 함축과는 아무 관련이 없고, 단지 콕이
    왼쪽에서 오른쪽으로 다시 작성하라고 지시하기 위한
    것이다. 오른쪽에서 왼쪽으로 다시 작성하려면 [rewrite <-] 명령을
...
Proof.
  ...
  (* (주의: 이 증명은 사실 [rewrite]를 사용하지 않고 완성할 수
     있다. 하지만 연습을 위해 [rewrite]를 사용하시오. ) *)
...
(** 물론 단순 계산과 다시 쓰기로 모든 것을 증명할 수 있는 것은 아니다. 일반적으로
    ...
    간략화를 못하게 막을 수 있다. 예를 들어, [simpl] 전술을 사용하여 위와 같이 
...
(** 그 이유는 [beq_nat]과 [+]의 두 정의들이 첫 번째 인자에 대하여
    [match]를 수행하면서 시작하기 때문이다. 그러나 여기에서 [+]의 첫
    번째 인자는 알려지지 않은 숫자 [n]이고 [beq_nat]의 첫 번째 인자는
    복합식 [n + 1]이다.  어느 것도 간략화될 수 없다.

    진행하기 위해 [n]의 가능한 형태들을 분리해서 고려할 필요가
    있다. 만일 [n]이 [O]이면 [beq_nat (n + 1) 0]의 최종 결과를 계산할
    수 있고 그 결과는 [fasle]라고 확인할 수 있다.  그리고 만일 어떤
    [n']에 대해 [n = S n']이라면 비록 숫자 [n + 1]이 정확히 무엇인지
    모르지만 최종 결과가 적어도 하나의 [S]를 가지고 시작한다고 계산할
    수 있다. 이 사실을 이용하면 다시 [beq_nat (n + 1) 0]이 [false]라고
    계산할 수 있다.

    콕이 [n = O]와 [n = S n'] 경우 별로 분리해서 고려하도록 지시하는
    전술은 [destruct]이다. *)
...
(** [destruct]는 _두 개_의 부분 목표들을 만들고, 콕으로 하여금 이
    정리를 받아들이도록 우리는 각각 증명해서 해야 한다. 주석 "[as [|
    n']]"을 _도입 패턴_이라 부른다. 각 부분 목표에서 콕이 도입할 변수
    이름들을 지정해준다. 이 주석의 대괄호들 사이에 [|]로 분리된 이름들
    _리스트의 리스트_가 온다. 위 증명의 경우 첫 번째 요소는 [O]
    생성자는 인자를 받지 않기 때문에 비어있다. 두 번째 요소는 [n']
    이름을 지정했는데 [S]는 단항 생성자이기 때문이다.

    두 번째와 세 번째 줄들에 [-] 부호들을 _불릿 표시들_이라고 부르는데
    각각 생성된 부분 목표에 해당하는 증명 부분들을 표시한다. 불릿
    다음에 오는 증명 스크립트는 하나의 부분 목표에 대한 증명 전체에
    해당한다.  이 예에서는 각 부분 목표는 [reflexivity]를 한 번
    사용해서 간략화하여 쉽게 증명할 수 있다. 예를 들어 첫 번째는 [(S
    n' + 1)]을 [S (n' + 1)]로 먼저 고쳐 작성하고 [beq_nat]을 펼친
    다음, [match]를 간략화하여 [beq_nat (S n' + 1) 0]을 [false]로
    간략화한다.
    ...
    [destruct] 전술은 모든 귀납적으로 정의한 자료형과 함께 사용할 수
...
(** 여기에서 [destruct]는 [as] 절을 가지고 있지 않은데 [destruct]로
    인해 생성된 어떠한 부분 경우들도 변수들과 연결될 필요가 없기
    때문이다. 그래서 이름을 지정할 필요가 없다. ([as [|]]나 [as []]와
    같이 작성할 수 있긴 하다.) 사실 _어떠한_ [destruct]에서도 [as] 절을
    생략할 수 있다. 그러면 콕은 변수 이름 들을 자동으로 채워 넣을 것이다.
    일반적으로 이것은 나쁜 스타일일로 간주한다. 왜냐하면 콕이 자동으로 선택하면
    혼동스러운 이름을 만들 수 있기 때문이다.

    부분 목표 안에서 [destruct] 명령을 내리면 한층 더 많이 성립해야 할
...
(** [reflexivity]의 각 쌍은 바로 위의 [destruct c]를 실행해서 생성된
    부분 목적들에 해당한다. *)


(** [-]와 [+] 외에도 [*] (별표)를 세 번째 종류의 불릿으로 사용할 수
...
(** 이 장을 마치기 전에 마지막 편리한 방법을 언급한다. 아마도 이미 봤을 것과 같이
    ...
    위 [plus_1_neq_0] 정리를 더 간단하게 증명할 수 있다. *)
...
(** 다음 주장을 증명하되 [destruct]을 사용할 때 불릿들을 가지고
...
(** 콕의 표기법 기호를 위해 _우선순위 단계_와 _결합성_을 지정할 수
    있다.  우선순위 단계 [n]은 [at level n]으로 작성하여 지정한다. 이
    것은 콕이 복잡한 식들의 구조를 분석할 때(파싱 할 때)
    필요하다. 결합성 설정은 동일한 기호가 여러 번 나타나는 식들을
    모호하지 않게 구조를 분석할 때 사용된다. 예를 들어 [+]와 [*]에
    대해 위에서 지정한 패러미터들은 [1+2*384]라는 식은
    [(1+((2*3)*4))]을 괄호 없이 짧게 작성한 것임을 분석하도록 도와준다.
    콕은 우선순위 단계로 0부터 100을 사용하고, _왼쪽_, _오른쪽_,
    _결합성 없음_을 사용한다. 이 것에 대한 더 많은 예제들을 나중에, 예를 들어
    리스트 장에서, 볼 것이다. 

    각 표기법 기호는 _표기법 유효 범위_와도 연관되어 있다. 콕은
    문맥에서 유효 범위를 추측한다. [S (O*O)]에 대해 [nat_scope]라고
    추측하고, 데카르트 곱(튜플) 타입 [bool*bool] (이후 장들에서 설명할
    것이다)에 대해 [type_scope]라고 알아낸다. 때때로 퍼센트 표기법을
    사용하여 [(x*y)%nat]라고 작성해서 콕이 유효 범위를 추측하는 것을
    도와줄 필요가 있고, 콕이 출력할 때 [%nat]을 사용하여 표기법이
    포함된 유효 범위를 표시하기도 한다.

    표기법 유효 범위들은 숫자 표기법([3], [4], [5], 등등)에도
    적용한다.  때때로 [0%nat]을 볼 수 있는데 [O] (이 장에서 사용한
    자연수 [0])이다.  [0%Z]는 정수 영(표준 라이브러리의 다른 부분에
    있는)이다.
...
(** 콕이 이 정의 검사할 때 [plus']는 "첫 번째 인자에 대해 감소"한다고
    설명할 것이다. 이 것은 인자 [n]에 대한 _구조적 재귀_를 수행한다는
    것이다.  즉, 오직 [n]의 더 작은 값들에 대해서만 재귀 함수를
    호출한다는 뜻이다. 이 것은 [plus']의 모든 호출은 궁극적으로 종료할
    것을 함축한다. 콕에서는 _모든_ [Fixpoint] 정의의 어떤 인자는
    "감소"해야 한다.
...
(** 구체적으로 이해하기 위해, 모든 입력들에 대해 _종료하는_ (가령,
    ...
    거절하는 의미 있는 [Fixpoint] 정의를 작성해보시오. *)
...
(** 이전 정리와 유사하지만 함수 [f]가 [f x = negb x] 성질을 가지고
    있다는 두 번째 가정을 갖춘 [negation_fn_applied_twice] 정리를
    기술하고 증명하시오 *)
...
(** 다음 정리를 증명하시오. (힌트: 접근 방법에 따라 약간 까다로울 수
    있다.  분명히 [destruct]와 [rewrite] 둘 다 필요할 것이지만 보이는
...
(** 자연수를 표현하는 다른 더 효율적인 방법을 생각해보자. 즉, 각 자연수를 영 또는
    ...
    (a) 첫째, 위의 설명에 부합하도록 타입 [bin]의 귀납적 정의를
        작성하시오.

    (힌트: 위의 [nat]의 정의를 살펴보면,

         Inductive nat : Type := | O : nat | S : nat -> nat.

    [O]와 [S]가 "의미"하는 것에 대해 아무것도 얘기하고 있지 않다. 단지
    "[O]는 [nat]이라 부르는 집합의 원소이고, [n]이 이 집합에 있으면 [S
    n]도 있다"라고 기술한다. [O]를 영으로 [S]를 다음 숫자/더하기 일로
    해석하는 것은 [nat] 값들을 _사용_하는 방법에서 정해진다. 이
    원소들을 가지고 무언가를 하는 함수를 작성하고, 이 원소들에 대해
    무언가를 증명하고, 등등과 같은 방법에 의해 정해진다. [bin]의
    정의에 수학적 의미를 부여하는 것은 그 다음에 작성할 함수들이기
    때문에 [bin]을 그에 맞추어 간단하게 정의해야 한다.)

    (b) 다음으로, 이항 숫자들에 대한 증가 함수 [incr]을 작성하고 이 숫자들을
        단항 숫자들로 변환하는 [bin_to_nt] 함수를 작성하시오.

    (c) 증가 하수와 이항 수를 단항 수로 변환하는 함수들에 대해 다섯
        개의 유닛 테스트로 [test_bin_incr1], [test_bin_incr2], 등을
        작성하시오. (콕에서 "유닛 테스트"는 그 동안 여러 정의들에 대해
        해온 것 처럼 단순히 [reflexivity]로 증명할 수 있는 특정
        [Example]이다. 예를 들어, 이항 수를 증가하고 그 다음에 단항
